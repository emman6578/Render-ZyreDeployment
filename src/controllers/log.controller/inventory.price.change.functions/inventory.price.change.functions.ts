// Interface for inventory price change analytics
export interface InventoryPriceChangeInsight {
...
export interface InventoryPriceStatistics {
  costPrice: {
    current: number;
    highest: number;
    lowest: number;
...
export interface InventoryPriceTrend {
  period: string;
...
export function calculatePriceInsights(
  history: any[]
): (InventoryPriceChangeInsight | null)[] {
...
export function analyzePriceTrends(history: any[]): InventoryPriceTrend[] {
  const trends: InventoryPriceTrend[] = [];
...
export function findBiggestChange(
  insights: (InventoryPriceChangeInsight | null)[],
  type: "increase" | "decrease"
) {
  const validInsights = insights.filter(
    (i) => i && i.changeType === type
  ) as InventoryPriceChangeInsight[];
...
export function generateRecommendations(
  statistics: InventoryPriceStatistics,
  trends: InventoryPriceTrend[],
  summary: any
): string[] {
  ...
  if (statistics.retailPrice.volatility > 50) {
    recommendations.push(
      "High inventory price volatility detected. Consider implementing more stable pricing strategy for inventory items."
    );
  }
  ...
  if (statistics.profitMargin.current < 20) {
    recommendations.push(
      "Current inventory profit margin is low. Review pricing strategy to improve profitability on inventory items."
    );
  }

  if (statistics.profitMargin.current > 100) {
    recommendations.push(
      "Very high inventory profit margin detected. Consider competitive pricing analysis for inventory items."
    );
  }
  ...
  if (recentTrend?.trend === "downward") {
    recommendations.push(
      "Recent downward inventory price trend observed. Monitor market conditions and supplier pricing."
    );
  }
  ...
  if (summary.totalChanges > 20) {
    recommendations.push(
      "Frequent inventory price changes detected. Consider establishing more consistent pricing intervals for inventory management."
    );
  }
  ...
  if (statistics.costPrice.volatility > 30) {
    recommendations.push(
      "High cost price volatility in inventory. Consider negotiating fixed-term contracts with suppliers."
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "Inventory price change pattern appears stable and within normal ranges."
    );
  }
...
export function calculatePriceStabilityScore(
  statistics: InventoryPriceStatistics
): number {
...
export function assessProfitMarginHealth(currentMargin: number): string {
  if (currentMargin < 10) return "Poor - Very low profit margin";
  if (currentMargin < 20) return "Fair - Below average profit margin";
  if (currentMargin < 40) return "Good - Healthy profit margin";
  if (currentMargin < 60) return "Very Good - Strong profit margin";
  if (currentMargin < 100) return "Excellent - High profit margin";
  return "Exceptional - Very high profit margin (review competitiveness)";
}

export function calculateBatchHealthScore(productAnalytics: any[]): number {
  if (productAnalytics.length === 0) return 0;

  const scores = productAnalytics.map(
    (p) => p.analytics.summary.priceStabilityScore
  );
...
export function generateBatchRecommendations(
  ...
  if (batchSummary.batchHealthScore < 60) {
    recommendations.push(
      "Batch shows high price volatility. Consider reviewing supplier agreements and pricing strategies."
    );
  }

  if (batchSummary.averageChangesPerProduct > 5) {
    recommendations.push(
      "High frequency of price changes detected. Consider implementing more stable pricing intervals."
    );
  }

  // Check for products with concerning trends
  const poorPerformers = productAnalytics.filter(
    (p) => p.analytics.summary.priceStabilityScore < 50
  );

  if (poorPerformers.length > 0) {
    recommendations.push(
      `${poorPerformers.length} product(s) show concerning price volatility. Focus on stabilizing these items first.`
    );
  }

  // Check overall profitability
  const lowMarginProducts = productAnalytics.filter(
    (p) => p.analytics.statistics.profitMargin.current < 20
  );

  if (lowMarginProducts.length > 0) {
    recommendations.push(
      `${lowMarginProducts.length} product(s) have low profit margins. Review pricing strategy for these items.`
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "Batch pricing appears stable and within acceptable ranges. Continue monitoring for any changes."
    );
  }
