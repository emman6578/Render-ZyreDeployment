...
export class PurchaseUpdateValidator {
  /**
   * Main validation function for purchase update requests
   */
  static async validatePurchaseUpdate(
    purchaseId: number,
    ...
    try {
      ...
      if (updateData.items) {
        await this.validatePurchaseItems(
          updateData.items,
      ...
      await this.validateBusinessRules(updateData, existingPurchase);
    } catch (error) {
   ...
   */
  private static validatePurchaseFields(updateData: PurchaseUpdateData): void {
    // Batch Number
   ...
   */
  private static validateDates(updateData: PurchaseUpdateData): void {
    const dateFields = [
   ...
   */
  private static validateStringFields(updateData: PurchaseUpdateData): void {
    const stringFields = [
  ...
  private static async validateForeignKeys(
    ...
    if (updateData.supplierId !== undefined) {
      if (
        !Number.isInteger(updateData.supplierId) ||
    ...
    if (updateData.districtId !== undefined) {
      if (
        !Number.isInteger(updateData.districtId) ||
   ...
   */
  private static async validatePurchaseItems(
    items: PurchaseItemUpdateData[],
    existingItems: any[]
  ): Promise<void> {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item._delete) {
        if (!item.id || !Number.isInteger(item.id) || item.id <= 0) {
          throw new Error(
            `${itemPrefix}: Valid item ID is required for deletion`
          );
        }
        ...
        if (!existingItem) {
          throw new Error(
            `${itemPrefix}: Item to delete not found in current purchase`
          );
        }
        continue;
      }
      ...
      // Validate product ID
      await this.validateItemProductId(item, itemPrefix);
      ...
      if (item.id !== undefined) {
        ...
        if (!existingItem) {
          throw new Error(
            `${itemPrefix}: Item to update not found in current purchase`
          );
        }
      }
    }
   ...
   */
  private static async validateItemProductId(
    item: PurchaseItemUpdateData,
    itemPrefix: string
  ): Promise<void> {
   ...
   */
  private static validateItemQuantities(
    item: PurchaseItemUpdateData,
    itemPrefix: string
  ): void {
    ...
    if (item.initialQuantity === 0) {
      throw new Error(
        `${itemPrefix}: Initial quantity must be greater than zero`
      );
    }

    if (item.initialQuantity > 1000000) {
      throw new Error(
        `${itemPrefix}: Initial quantity cannot exceed 1,000,000`
      );
    }
    ...
    if (item.currentQuantity > 1000000) {
      throw new Error(
        `${itemPrefix}: Current quantity cannot exceed 1,000,000`
      );
    }
    ...
    if (item.currentQuantity > item.initialQuantity) {
      throw new Error(
        `${itemPrefix}: Current quantity (${item.currentQuantity}) cannot exceed initial quantity (${item.initialQuantity})`
      );
    }
  }
   ...
   */
  private static validateItemPricing(
    item: PurchaseItemUpdateData,
    itemPrefix: string
  ): void {
    ...
    if (costDecimalPlaces > 2) {
      throw new Error(
        `${itemPrefix}: Cost price can have maximum 2 decimal places`
      );
    }
    ...
    // Check decimal places (max 2)
    const retailDecimalPlaces = (
      item.retailPrice.toString().split(".")[1] || ""
    ).length;
    if (retailDecimalPlaces > 2) {
      throw new Error(
        `${itemPrefix}: Retail price can have maximum 2 decimal places`
      );
    }
    ...
    if (item.retailPrice < item.costPrice) {
      throw new Error(
        `${itemPrefix}: Retail price (${item.retailPrice}) is lower than cost price (${item.costPrice}). Please verify pricing.`
      );
    }
  }
   ...
   */
  private static validateItemStatus(
    item: PurchaseItemUpdateData,
    itemPrefix: string
  ): void {
    if (item.status !== undefined) {
      ...
      if (!validStatuses.includes(item.status)) {
        throw new Error(
          `${itemPrefix}: Status must be one of: ${validStatuses.join(", ")}`
        );
      }
    }
  }
   ...
   */
  private static validateItemUpdateReason(
    item: PurchaseItemUpdateData,
    itemPrefix: string
  ): void {
    if (item.lastUpdateReason !== undefined) {
      ...
      if (item.lastUpdateReason.length > 500) {
        throw new Error(
          `${itemPrefix}: Update reason cannot exceed 500 characters`
        );
      }
    }
  }
   ...
   */
  private static validateNoDuplicateProducts(
    items: PurchaseItemUpdateData[]
  ): void {
    ...
    if (uniqueDuplicates.length > 0) {
      throw new Error(
        `Duplicate product IDs found in request: ${uniqueDuplicates.join(", ")}`
      );
    }
  }
  ...
  private static async validateBusinessRules(
    ...
    if (updateData.batchNumber && updateData.supplierId) {
      const duplicateBatch = await prisma.purchase.findFirst({
        where: {
    ...
    if (updateData.status === "ACTIVE" && updateData.expiryDate) {
      ...
      if (expiryDate < today) {
        throw new Error(
          "Cannot set an active purchase with expiry date in the past"
        );
      }
    }
  }
 ...
 */
export const validatePurchaseUpdateRequest = async (
  purchaseId: number,
  updateData: PurchaseUpdateData,
  userId: number
): Promise<void> => {
  return await PurchaseUpdateValidator.validatePurchaseUpdate(
    purchaseId,
    updateData,
    userId
  );
