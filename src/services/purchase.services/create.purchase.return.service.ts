...
} from "src/types/purchase.types";

const prisma = new PrismaClient();

export const purchase_return_create = async (
  ...
  try {
    ...
    const originalPurchaseItem = await prisma.purchaseItems.findUnique({
    ...
    if (originalPurchaseItem.batchId !== originalPurchaseId) {
      throw new Error(
        "Purchase item does not belong to the specified purchase"
      );
    }
    ...
    if (originalPurchase.status !== "ACTIVE") {
      throw new Error(
        `Cannot return items from ${originalPurchase.status.toLowerCase()} purchase`
      );
    }
    ...
    if (returnQuantity > originalPurchaseItem.currentQuantity) {
      throw new Error(
        `Return quantity (${returnQuantity}) cannot exceed current quantity (${originalPurchaseItem.currentQuantity})`
      );
    }

    // Check if there are already existing returns for this item
    const existingReturns = await prisma.purchaseReturn.aggregate({
      where: {
    ...
    const recentDuplicateReturn = await prisma.purchaseReturn.findFirst({
      where: {
        originalPurchaseItemId: originalPurchaseItemId,
    ...
    if (recentDuplicateReturn) {
      throw new Error(
        "Duplicate return request detected. A similar return with the same quantity and reason was already submitted recently."
      );
    }

    const totalReturnedQuantity = existingReturns._sum.returnQuantity || 0;
    const availableForReturn =
      originalPurchaseItem.initialQuantity - totalReturnedQuantity;

    if (returnQuantity > availableForReturn) {
      throw new Error(
        `Return quantity (${returnQuantity}) exceeds available quantity for return (${availableForReturn}) All quantity from the initial quantity of this item/s are now marked as returned.`
      );
    }

    const calculatedReturnPrice =
      Number(originalPurchaseItem.costPrice) * returnQuantity;

    // ─── MAIN TRANSACTIONAL LOGIC ──────────────────────────────────────────
    const result = await prisma.$transaction(async (tx) => {
      const referenceNumber = await generateRefNumber(prisma, 6, "PR");
    ...
    const createdPurchaseReturn: any = {
