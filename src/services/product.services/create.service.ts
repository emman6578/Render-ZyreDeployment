...
export interface CreateProductRequest {
  ...
  genericId: string;
  brandId: string;
  categoryIds: string | string[]; // Can be single string or array of strings
  companyId: string;
...
const validateProductData = async (data: CreateProductData) => {
  ...
  if (
    ...
    throw new Error(
      "All required fields must be provided: genericId, brandId, categoryIds (array), companyId, safetyStock, averageCostPrice, averageRetailPrice, createdById"
    );
  }

  // 2. Check for valid numeric values (not NaN)
  if (
    isNaN(data.genericId) ||
    isNaN(data.brandId) ||
  ...
  if (existingProducts.length > 0) {
    ...
    for (const existingProduct of existingProducts) {
      ...
      if (
        ...
        throw new Error(
          "Product with the same generic, brand, company, and categories combination already exists"
        );
      }
    }
  }
  ...
  await validateForeignKeys(
    data.genericId,
    data.brandId,
    data.categoryIds, // Pass array of category IDs
    data.companyId
  );
};

// New bulk creation function
export const create_products_bulk = async (dataArray: CreateProductData[]) => {
  // First, validate all items without creating anything
  for (let i = 0; i < dataArray.length; i++) {
    try {
      await validateProductData(dataArray[i]);
    } catch (error: any) {
  ...
  for (let i = 0; i < dataArray.length; i++) {
    ...
    if (combinations.has(combination)) {
      throw new Error(
        `Duplicate combination found within the array at item ${
          i + 1
        }: generic ${dataArray[i].genericId}, brand ${
          dataArray[i].brandId
        }, company ${dataArray[i].companyId}`
      );
    }
    ...
    for (const data of dataArray) {
      ...
      const product = await tx.product.create({
        data: {
          ...
          lastUpdateReason:
            data.lastUpdateReason?.trim() || "Initial product creation",
