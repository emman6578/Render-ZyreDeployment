...
const calculateRunningBalance = (transactions: any[]) => {
  ...
  return transactions.map((transaction) => {
    ...
    const quantityChange =
      (transaction.quantityIn || 0) - (transaction.quantityOut || 0);
    runningBalance += quantityChange;

    return {
      ...transaction,
      balance: runningBalance, // Add computed balance field
    };
  });
};

// New function to get complete transaction history with balance
export const getProductTransactionHistory = async (productId: number) => {
  ...
  const currentInventoryBalance = await prisma.inventoryItem.aggregate({
    where: {
      productId: productId,
  ...
  const summary = {
    ...
    calculatedBalance:
      transactionsWithBalance.length > 0
        ...
        : 0,
    totalInbound: allTransactions.reduce(
      (sum, t) => sum + (t.quantityIn || 0),
      0
    ),
    totalOutbound: allTransactions.reduce(
      (sum, t) => sum + (t.quantityOut || 0),
      0
    ),
    balanceValidation: {
      calculatedBalance:
...
export const readByid = async (
  ...
  const beginningInventory =
    transactionsWithBalance.length > 0 ? transactionsWithBalance[0].balance : 0;
