...
export async function validateCreatePurchaseBatch(
  data: any,
  prisma: PrismaClient
) {
  ...
  for (const batch of data.batches) {
    ...
    for (const item of batch.items) {
      ...
      const existingBatch = await prisma.purchase.findFirst({
        where: {
      ...
      if (existingBatch) {
        throw new Error(
          `Batch number '${batch.batchNumber}'. Already exists, Please use a unique batch number.`
        );
      }
    }
  }
